...
public class CategoryUseCase implements ICategoryServicePort {
    ...
    public Category addCategory(Category category) {
        ...
        if (category.getName().trim().isEmpty()) {
            throw new EmptyCategoryNameException(DomainConstants.Field.NAME.toString());
        }
        if (category.getName().length() > DomainConstants.MAX_CHARACTERS_CATEGORY_NAME) {
            throw new LongCategoryNameException(DomainConstants.Field.NAME.toString());
        }
        if(Boolean.TRUE.equals(categoryPersistencePort.existsByName(category.getName()))) {
            throw new CategoryAlreadyExistsException(DomainConstants.Field.CATEGORY.toString());
        }

        if (category.getDescription().trim().isEmpty()) {
            throw new EmptyCategoryDescriptionException(DomainConstants.Field.DESCRIPTION.toString());
        }
        if (category.getDescription().length() > DomainConstants.MAX_CHARACTERS_CATEGORY_DESCRIPTION) {
            throw new LongCategoryDescriptionException(DomainConstants.Field.DESCRIPTION.toString());
        }
